...
*** load path
    #+BEGIN_SRC emacs-lisp
      (dolist (item '("lisp" "site-lisp"))
        (add-to-list 'load-path
                     (expand-file-name item creature-dir)))
    #+END_SRC
...
** paredit mode
   ...
   config space before "()" when type "("
   #+BEGIN_SRC emacs-lisp
     (defun paredit/space-for-delimiter-p (endp delm)
       (or (member 'font-lock-keyword-face
...
** emmet-mode
   #+BEGIN_SRC emacs-lisp
     ...
     (dolist (state '(insert emacs))
       (evil-define-key state emmet-mode-keymap
